...
// ClientIPPort 尽最大努力实现获取客户端 IP 和 port 的算法
...
/*
	...
	server{
		...
		location / {
			...
			# 不可信，但可以打印日志，跟踪跳转，也可以将 最左边的ip 对比 X-Real-IP,不相同则可以认为是攻击者
...
*/
func ClientIPPort(r *http.Request) (ip string, port string) {
	...
	port = strings.TrimSpace(r.Header.Get("X-Real-PORT"))
	if ip == `` && port == `` {
		var err error
